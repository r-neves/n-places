...
export class NotionAPIRestaurantsRepository implements RestaurantsRepository {
	...
	async getDBLastUpdatedDate(): Promise<Date> {
		return await NotionAPIClient.fetchDBLastUpdatedDate(process.env.RESTAURANTS_DB_ID!);
	}
	
	async getRestaurants(lastModifiedDate: Date): Promise<RepoRestaurant[]> {
		const restaurants: RepoRestaurant[] = await NotionAPIClient.fetchPlacesFromNotion(process.env.RESTAURANTS_DB_ID!, lastModifiedDate);
		...
		await Promise.all(coordinatePromises).then((coordinates) => {
			coordinates.forEach(c => {
				...
				rowMetadataUpdatePromises.push(
					NotionAPIClient.patchPlaceMetadata(process.env.RESTAURANTS_DB_ID!, restaurants[c.index].id, metadata)
				);
		...
		if (coordinatePromises.length > 0) {
			console.info("Finished updating %d coordinates", coordinatePromises.length);
		}
